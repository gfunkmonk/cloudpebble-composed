...
class Migration(DataMigration):
        ...
        for build in orm.BuildResult.objects.iterator():
            if build.total_size is not None:
                result_size = orm.BuildSize.objects.create(build=build,
                                                           ...
                                                           resource_size=build.resource_size,
                                                           worker_size=build.worker_size)

    def backwards(self, orm):
        "Write your backwards methods here."
        for size in orm.BuildSize.objects.filter(platform='aplite'):
            build = size.build
            build.total_size = size.total_size
            build.binary_size = size.binary_size
            build.resource_size = size.resource_size
            build.worker_size = size.worker_size
            build.save()
            size.delete()
    ...
    models = {
        ...
        'ide.buildresult': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'c33c87c4-0d14-4c93-9252-3f9233c8b6be'", 'max_length': '36'}),
        ...
        'ide.project': {
            ...
            'app_uuid': ('django.db.models.fields.CharField', [], {'default': "'26f8e5e6-c8b5-4b2b-bca4-8b3a25d22d45'", 'max_length': '36', 'null': 'True', 'blank': 'True'}),
