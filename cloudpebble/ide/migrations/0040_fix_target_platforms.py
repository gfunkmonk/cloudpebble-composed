...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """For each ResourceVariant, set its target_platforms to its resource_file's target_platforms"""
        for resource_identifier in orm.ResourceIdentifier.objects.all():
            resource_identifier.target_platforms = resource_identifier.resource_file.target_platforms
            resource_identifier.save()


    def backwards(self, orm):
        """For each ResourceFile, set it to target all of the platforms that its resource identifiers target,
        or null if none of its identifiers have targets."""
        for resource_file in orm.ResourceFile.objects.all():
            platforms = None
            for resource_identifier in orm.ResourceIdentifier.objects.filter(resource_file=resource_file):
                id_platforms = json.loads(resource_identifier.target_platforms)
                if id_platforms is not None:
                    if platforms is None:
                        platforms = set()
                    platforms.update(id_platforms)
            if platforms is not None:
                resource_file.target_platforms = json.dumps(list(platforms))
            else:
                resource_file.target_platforms = json.dumps(None)
            resource_file.save()
    ...
    models = {
        ...
        'ide.buildresult': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'49b98229-7956-4c53-9a5d-469662504d21'", 'max_length': '36'})
        },
        'ide.buildsize': {
        ...
        'ide.project': {
            ...
            'app_uuid': ('django.db.models.fields.CharField', [], {'default': "'322a65a1-a1db-4310-9917-2240b761083b'", 'max_length': '36', 'null': 'True', 'blank': 'True'}),
