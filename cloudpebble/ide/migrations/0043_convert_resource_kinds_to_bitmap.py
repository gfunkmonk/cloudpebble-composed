...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Convert PNGs and PBIs to bitmaps """
        for resource_file in orm.ResourceFile.objects.all():
            if resource_file.kind in ('png', 'pbi'):
                if resource_file.kind == 'pbi':
                    # Set 1-bit memory format for all resource-IDs for PBIs.
                    identifiers = orm.ResourceIdentifier.objects.filter(resource_file=resource_file)
                    for resource_identifier in identifiers:
                        resource_identifier.memory_format = '1Bit'
                        resource_identifier.save()
                resource_file.kind = 'bitmap'
                resource_file.save()


    def backwards(self, orm):
        """ This migration will loose information if CloudPebble has been used since the forward migration,
         since it is not possible to determine if a thing was a png or a pbi if its resource IDs are ambiguous."""
        for resource_file in orm.ResourceFile.objects.all():
            if resource_file.kind == 'bitmap':
                identifiers = orm.ResourceIdentifier.objects.filter(resource_file=resource_file)
                for resource_identifier in identifiers:
                    # If any resource ID has a memory format that isn't 1 bit, it was a PNG
                    if resource_identifier.memory_format != '1Bit':
                        resource_file.kind = 'png'
                        break
                else:
                    # Otherwise, all formats are 1-bit, so it was a PBI
                    resource_file.kind = 'pbi'
                resource_file.save()
    ...
    models = {
        ...
        'ide.buildresult': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'6fc5613b-a8bf-4ef2-bb75-8937a5db362d'", 'max_length': '36'})
        },
        'ide.buildsize': {
        ...
        'ide.project': {
            ...
            'app_uuid': ('django.db.models.fields.CharField', [], {'default': "'3c67d0b9-5d89-4238-9fb4-2333a95df88f'", 'max_length': '36', 'null': 'True', 'blank': 'True'}),
        ...
        'ide.usersettings': {
            ...
            'whats_new': ('django.db.models.fields.PositiveIntegerField', [], {'default': '19'})
        }
    }
