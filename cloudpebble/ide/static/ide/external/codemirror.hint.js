...
(function(mod) {
  ...
  Completion.prototype = {
    ...
    showWidget: function(data) {
      ...
      CodeMirror.signal(data, "shown", this.widget.hints);
      CodeMirror.signal(data, "select", data.list[0]);
  ...
  function Widget(completion, data) {
    ...
    if (overlapY > 0) {
      ...
      if (curTop - height > 0) { // Fits above cursor
        hints.style.top = (top = curTop - height - 20) + "px";
        below = false;
      } else if (height > winH) {
        hints.style.height = (winH - 5) + "px";
        hints.style.top = (top = pos.bottom - box.top - 20) + "px";
  ...
  CodeMirror.registerHelper("hint", "fromList", function(cm, options) {
    ...
    var found = [];
    for (var i = 0; i < options.words.length; i += 1) {
